...
import { fetchCourses, deleteCourse, togglePublish } from '../../features/courses/courseSlice';
...
export default function AdminDashboard() {
    ...
    const loadEnrollments = useCallback(async () => {
        ...
        try {
            ...
            const allCourses = await courseApi.getCourses(auth.token, false);
            
            // Get all enrollments
            const allEnrollments = [];
            for (const course of allCourses) {
                try {
                    const progress = await progressApi.getCourseProgress(course._id, auth.token);
                    if (Array.isArray(progress)) {
                        allEnrollments.push(...progress.map(p => ({
                            ...p,
                            course: course,
                            student: p.user // Assuming the progress has user info
                        })));
                    }
                } catch (err) {
    ...
    useEffect(() => {
        ...
        loadEnrollments();
    }, [dispatch, loadStudentsAndApprovals, loadEnrollments]);
    ...
    // Helper to check if delete action is in progress for THIS or ANY student
        ...
        <div style={{ display: 'flex', minHeight: '100vh', background: SOFT_BG }}>
            ...
            <main style={{ flex: 1, padding: 32, maxWidth: '100%', overflowX: 'hidden' }}>
                                    ...
                                    <table style={{ width: '100%', minWidth: '900px', borderCollapse: 'collapse' }}>
                                        ...
                                        <tbody>
                                            {filteredStudents.map((student) => {
                                                const { text, color, bg } = getStatusDisplay(student.status);
                                                const isCurrentStudentDeleting = deletingStudentId === student._id;
                                                ...
                                                return (
                                                    <tr key={student._id} style={{ borderTop: `1px solid ${SOFT_BORDER_COLOR}` }}>
                                                        ...
                                                        <td style={{ padding: '12px 20px', fontSize: 14 }}>
                                                            <span style={statusBadgeStyle(color, bg)}>
                                                                {text}
                                                            </span>
                                                        </td>
                                                        <td style={{ padding: '12px 20px', fontSize: 14, textAlign: 'center' }}>
                                                            {(student.status === 'pending' && !isCurrentStudentDeleting) && (
                                                            ...
                                                            {(student.status === 'approved' || student.status === 'rejected') && (
                                                                <button
                                                                    ...
                                                                    disabled={isCurrentStudentDeleting}
                                                                >
                ...
                {activeTab === 'enrollments' && (
                    <CourseEnrollments token={auth?.token} />
                )}
